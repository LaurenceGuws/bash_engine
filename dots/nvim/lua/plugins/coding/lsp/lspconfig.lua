return {
  ...
  config = function()
    ...
    local on_attach = function(client, bufnr)
      ...
      -- Diagnostics
      keymap(bufnr, "n", "[d", "<cmd>lua vim.diagnostic.goto_prev({severity = {min = vim.diagnostic.severity.WARN}})<CR>", opts)
      keymap(bufnr, "n", "]d", "<cmd>lua vim.diagnostic.goto_next({severity = {min = vim.diagnostic.severity.WARN}})<CR>", opts)
      keymap(bufnr, "n", "[e", "<cmd>lua vim.diagnostic.goto_prev({severity = vim.diagnostic.severity.ERROR})<CR>", opts)
      keymap(bufnr, "n", "]e", "<cmd>lua vim.diagnostic.goto_next({severity = vim.diagnostic.severity.ERROR})<CR>", opts)
    ...
    for server_name, config in pairs(server_configs) do
      if lspconfig[server_name] then
        ...
        if not success then
          vim.notify(string.format("Failed to set up LSP server %s: %s", server_name, err), vim.log.levels.ERROR)
        end
      else
    ...
    vim.api.nvim_create_autocmd({"BufReadPost", "BufNewFile"}, {
      callback = function()
        ...
        if filename:match("swagger") or filename:match("openapi") or
           (extension == "json" or extension == "yaml" or extension == "yml") and
           (vim.fn.getline(1):match("swagger") or vim.fn.getline(1):match("openapi")) then
        ...
        if vim.fn.findfile("docker-compose.yml", ".;") ~= "" and
           vim.fn.readfile(vim.fn.findfile("docker-compose.yml", ".;"))
           :join():match("kafka") or
           vim.fn.readfile(vim.fn.findfile("docker-compose.yml", ".;"))
           :join():match("rabbitmq") then
